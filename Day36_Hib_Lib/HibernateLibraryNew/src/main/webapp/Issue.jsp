		...
		if (sbook != null) {
			for (int i = 0; i < sbook.length; i++) {
				...
				if (res.equals("yes")) {
					...
					Date idate = new Date();
					  java.sql.Date sqlDate = new java.sql.Date(idate.getTime());
	%>
	<jsp:useBean id="bean1" class="com.java.lib.TransBook" />
	<jsp:setProperty property="userName" name="bean1" value="<%=user%>" />
	<jsp:setProperty property="bookId" name="bean1" value="<%=bid%>" />
	<jsp:setProperty property="fromDate" name="bean1" value="<%=sqlDate%>" />
